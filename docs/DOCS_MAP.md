...
| Understand how a query compiles to a Spark plan | [`DESIGN.md`](../DESIGN.md) — start at §4 (architecture of semanticdf) |
...
2. [`examples/starter/`](../examples/starter/) — read its README, then run it (`mvn scala:run -DmainClass=com.example.starter.Main` from the example dir)

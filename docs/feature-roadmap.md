...
**Last updated:** v0.1.3 shipped (Jackson Scala module wiring, REST test infra fixes, CLI consumer debut at `examples/cli-consumer/`); plus two post-tag fixes on `main` (PR `#61`: `Introspect.handle()` `warnings` end-to-end + `field_inventory.skipped` agrees with `warnings.length`; PR `#62`: clean `toJoinYaml` placeholder names — `_model` is gone for `id`-typed columns, and longer entity suffixes like `_uuid` / `_code` / `_key` strip cleanly). 8 templates shipping (`cli-consumer` added in v0.1.3); `sdf` CLI is the project's first real consumer.
...
- **2026-07:** Post-v0.1.3: PR `#61` (wiring `# WARN:` lines through `Introspect.handle()`; `field_inventory.skipped` agrees with `warnings.length`). PR `#62` (clean entity placeholder names in `Introspector.toJoinYaml` — bare `id` → `id_model:`, `_uuid`/`_code`/`_key` suffixes strip cleanly). Library at 329 tests; MCP at 72 tests.

...
private[semanticdf] trait SemanticTableMutation { self: SemanticTable =>
  ...
  private[semanticdf] def joinRollups(other: SemanticTable): List[io.semanticdf.rollup.Rollup] =
    this.rollups ++ other.rollups  // concatenate; both sides' rollups are preserved
    ...
    if (leftKeysIn.isEmpty && rightKeysIn.isEmpty && onExprStringIn.isEmpty) {
      ...
      if (lk.nonEmpty || rk.nonEmpty || sql.isDefined) {
        ...
        return new SemanticTable(
          ...
          salt = joinSalt(other), rollups = joinRollups(other),
        )
      }
    }
    new SemanticTable(

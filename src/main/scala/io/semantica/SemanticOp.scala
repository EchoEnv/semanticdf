...
object SemanticOp {
  ...
  private[semantica] def rootModel(op: SemanticOp): Option[SemanticTableOp] = op match {
...
private[semantica] final class JoinSide(
    private[semantica] val sideName: String,
    private[semantica] val df: DataFrame,
    private[semantica] val dims: Map[String, Dimension],
    private[semantica] val captured: scala.collection.mutable.Map[String, Boolean],
) {
...
final case class SemanticJoinOp(
  ...
  private[semantica] lazy val mergedModel: MergedSemanticModel = {
...
private[semantica] final class ClassificationScope(
    df: DataFrame,
    knownMeasures: scala.collection.Set[String],
) extends SemanticScope {
  private[semantica] val referencedMeasures = scala.collection.mutable.Set.empty[String]

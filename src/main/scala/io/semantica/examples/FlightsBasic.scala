...
/** Example 01 — Basic group-by and calc measures
  *
  * Run with: mvn scala:run -DmainClass=io.semantica.examples.FlightsBasic
  * Or submit to Spark: spark-submit target/semantica_2.13-*.jar
  */
object FlightsBasic {
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      // Also show the semantica plan (no Spark compile)
      println("\n=== Semantica plan (explain) ===")

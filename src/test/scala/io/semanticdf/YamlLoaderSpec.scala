...
class YamlLoaderSpec extends AnyFunSuite with SparkSessionFixture with FlightsFixture {
  ...
  test("YamlLoader.streamingConfig parses the optional `streaming:` block from a YAML model") {
    // Operator's companion parser: reads the streaming metadata that
    // accompanies a model in the same YAML file. The library never starts
    // a query from this — the operator does that, with this typed config.
    val f = File.createTempFile("yaml-streaming-cfg", ".yml")
    f.deleteOnExit()
    val w = new PrintWriter(f)
    try {
      w.write(
        """rate_events:
          ...
          |    output_mode: update
          |    checkpoint_location: /tmp/chkpt/rate
          |    output_sink:
          |      kind: parquet
          |      path: /tmp/out/rate
          |""".stripMargin)
    ...
    val cfg = YamlLoader.streamingConfig(f.getAbsolutePath, "rate_events")
    assert(cfg.isDefined, "expected Some(StreamingConfig)")
    val c = cfg.get
    c.window.map(_.column)    shouldBe Some("timestamp")
    c.window.map(_.duration)  shouldBe Some("5 minutes")
    c.watermark.map(_.column) shouldBe Some("timestamp")
    c.watermark.map(_.delay)  shouldBe Some("10 minutes")
    c.outputMode              shouldBe "update"
    c.checkpointLocation      shouldBe Some("/tmp/chkpt/rate")
    c.outputSink              shouldBe OutputSink.Parquet("/tmp/out/rate")
  }

  test("YamlLoader.streamingConfig returns None when the model has no streaming block") {
    val f = File.createTempFile("yaml-no-streaming", ".yml")
    f.deleteOnExit()
    val w = new PrintWriter(f)
    try {
      w.write(
    ...
    val cfg = YamlLoader.streamingConfig(f.getAbsolutePath, "batch_model")
    cfg shouldBe None
  }

  test("YamlLoader.streamingConfig throws when the streaming block is malformed") {
    val f = File.createTempFile("yaml-bad-streaming", ".yml")
    f.deleteOnExit()
    val w = new PrintWriter(f)
    try {
      w.write(
    ...
    val ex = intercept[IllegalArgumentException] {
      YamlLoader.streamingConfig(f.getAbsolutePath, "bad_model")
    }
    assert(ex.getMessage.contains("streaming"),
      s"Expected streaming error, got: ${ex.getMessage}")
  }
}

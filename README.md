...
```json
{
  "mcpServers": {
    "semanticdf": {
      ...
      "args": [
        "-jar",
        "/path/to/semanticdf-mcp/target/semanticdf-mcp_2.13-0.1.5.jar",
        "--models",
        "/path/to/your/models",
        "--data",
...
Verified green on both Spark lines (335 library tests + 72 MCP tests = 407 total,
on each):
...
- **[`docs/known-limitations.md`](docs/known-limitations.md)** — current scope & guardrails for v0.1.5 (batch-only, per-session security, symmetric join keys, etc.) with workarounds and roadmap hints. Read before first consumer.

...
class PortableQueryResultSpec extends AnyFunSuite with Matchers {
  ...
  // -- ResultEncoder contract (test the trait shape, not a specific impl) --

  test("ResultError sealed ADT has 3 cases: ColumnMissing, ValueDecodeFailed, ShapeMismatch") {
    val cases: List[ResultError] = List(
      ResultError.ColumnMissing("x"),
      ResultError.ValueDecodeFailed("y", "bad type"),
      ResultError.ShapeMismatch("mismatch"),
    )
    cases.foreach { e =>
      val out = new java.io.ByteArrayOutputStream()
      val oos = new java.io.ObjectOutputStream(out)
      ...
      oos.close()
      val ois = new java.io.ObjectInputStream(new java.io.ByteArrayInputStream(out.toByteArray))
      val back = ois.readObject().asInstanceOf[ResultError]
      ois.close()
      back shouldBe e
    }
  }

# sdf — the semanticdf CLI

`sdf` is a first-class Maven module (`semanticdf-cli`) — a thin HTTP+JSON
client for the [semanticdf](https://github.com/EchoEnv/semanticdf) REST
...
**Pedagogical note.** `sdf` deliberately depends on **zero** `semanticdf-*`
modules. This proves the REST surface is the contract — any HTTP+JSON
client can drive the platform. To verify:

```bash
mvn -q -pl semanticdf-cli dependency:tree | grep semanticdf-   # empty
```
...
```bash
# From the repo root
mvn -q -pl semanticdf-cli install -DskipTests

# Add the wrapper to your PATH (optional)
ln -s "$(pwd)/semanticdf-cli/bin/sdf" /usr/local/bin/sdf
```
...
sdf audit-tail                      show recent audit events (PLATFORM RESTATE — durable)
...
`sdf audit-tail` reads the platform's audit log. Two surfaces exist in this
...
`sdf audit-tail` therefore **requires** `--restate-url <base>` or
...
standalone audit endpoint is not exposed by `sdf` — that's the next
...
sdf audit-tail --limit 10

# All activity for a specific tenant in a window.
sdf audit-tail --tenant acme --since 2026-08-01T00:00:00Z --until 2026-08-12T00:00:00Z

# Raw envelope (for piping to jq).
sdf audit-tail --limit 5 --json | jq '.output[]'
```
...
echo 's3cret' > /tmp/sdf.tok && chmod 600 /tmp/sdf.tok
sdf list --token-file /tmp/sdf.tok
...
## Why a separate module, not bundled in semanticdf-mcp
...
only on jackson-databind + scala-library — no Spark, no semanticdf library,
no MCP SDK.
